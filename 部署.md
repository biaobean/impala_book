...
将hive/lib下MySQL的jar包拷贝到fe/target/dependency/下，比如：

```bash
cp $HIVE_HOME/lib/mysql-connector-java-5.1.30.jar fe/target/dependency/
```
...
scp impala-release.tar.bz2 hadoop@hadoop002:/opt/impala/

scp impala-release.tar.bz2 hadoop@hadoop003:/opt/impala/

scp impala-release.tar.bz2 hadoop@hadoop004:/opt/impala/
